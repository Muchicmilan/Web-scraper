...
import { ScraperConfigurationModel, ScrapedDataItemModel, EngineSettingsModel } from "./scrape-engine-schema.js";
...
export const handleUpdateConfiguration = async (req, res) => {
    try {
        ...
        const updatedConfig = await ScraperConfigurationModel.findByIdAndUpdate(id, updateData, { new: true, runValidators: true });
        if (!updatedConfig) {
            res.status(404).json({ error: "Configuration not found." });
            return;
        }
...
export const handleRunMultipleScrapeJobs = async (req, res) => {
...
export const handleGetEngineSettings = async (req, res) => {
    try {
        let settings = null;
...
export const handleUpdateEngineSettings = async (req, res) => {
    try {
        ...
        const updatedSettings = await EngineSettingsModel.findOneAndUpdate({ singleton: true }, { $set: { maxPoolSize, minPoolSize, idleTimeoutMs, retryLimit } }, { new: true, upsert: true, runValidators: true }).lean();
