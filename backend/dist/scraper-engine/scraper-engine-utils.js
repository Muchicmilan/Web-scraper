...
export async function navigatePage(browser, url) {
...
export function browserProcessListItem(listItemElement, primaryMappings, pageUrl) {
...
export async function extractDetailData(page, config) {
    ...
    try {
        const extractedData = await page.evaluate((selector, mappings, currentUrl) => {
            function set(obj, path, value) {
                const keys = Array.isArray(path) ? path : path.split('.');
                let current = obj;
                for (let i = 0; i < keys.length - 1; i++) {
            ...
            function extractData(targetElement, mappings, pageUrl) {
                if (!targetElement)
                    return {};
                const resultData = {};
        ...
        }, itemSelectorToUse, mappingsToUse, pageUrl);
