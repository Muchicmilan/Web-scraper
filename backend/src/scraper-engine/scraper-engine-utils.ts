...
*/
export function extractLinks(htmlContent: string, baseURL: string): URL[] {
...
*/
export function parseHTML(html: string, url: string, options: ScrapeOptions): ScrapedData | null {
...
export function isKeywordFound(content: ScrapedData, keywords: string[]): boolean {
    ...
    if (!searchableContent.trim()) return false;
