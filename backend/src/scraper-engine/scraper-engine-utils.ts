...
import { ExtractedData, SingleItemOutcome } from "./scraper-engine-types.js";
...
function extractSameDomainLinks(htmlContent: string | null, baseUrl: string): string[] {
                ...
                if (mapping.extractFrom === 'attribute') {
                    if (mapping.attributeName) {
                        ...
                        if (value && (mapping.attributeName === 'href' || mapping.attributeName === 'src')) {
                            try { value = new URL(value, pageUrl).href; }
                            catch (e) { console.warn(`[Utils:extractData] Could not resolve URL: "${value}" on "${pageUrl}"`); }
                        }
                    }
...
export async function scrapeDetailInline(
    detailUrl: string,
    ...
    try {
        ...
        console.log(`[Service:scrapeDetailInline] DEBUG: Does .prose exist BEFORE exclusions? ${detailPage('.prose').length}`);
        ...
        if (allExclusions.length > 0) {
            try { detailPage(allExclusions.join(', ')).remove(); }
            catch (exError) { console.warn(`[Service:scrapeDetailInline] Error applying exclusions on ${detailUrl}: ${exError}`); }
        }
        console.log(`[Service:scrapeDetailInline] DEBUG: Does .prose exist AFTER exclusions? ${detailPage('.prose').length}`);
        ...
        console.log(`[Service:scrapeDetailInline] DEBUG: detailContentElement found? ${detailContentElement.length}`);
        ...
        console.log(`[Service:scrapeDetailInline] DEBUG: Using mappings from source: ${mappingSource}. Mapping count: ${mappingsToUse.length}`);
        console.log(`[Service:scrapeDetailInline] DEBUG: Mappings being used:`, JSON.stringify(mappingsToUse)); // Log the actual mappings
       

        const extractedData = extractDataFromElement(
...
export async function processSingleListItem(
    itemElement: cheerio.Cheerio<any>,
    index: number,
    config: IScraperConfiguration,
    startUrl: string,
    ...
    const outcomes: SingleItemOutcome[] = [];
